    ...
    <div className="w-24 md:w-72 bg-white border-r border-slate-50 flex flex-col h-full sticky top-0">
      <div className="p-8">
        <div className="flex items-center space-x-3 mb-12">
          ...
          <span className="hidden md:block text-xl font-bold tracking-tight text-slate-900">Patron<span className="text-slate-400">OS</span></span>
        </div>
        ...
        <nav className="space-y-2">
          {navItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center px-4 py-3 text-sm font-medium rounded-2xl transition-all duration-300 group ${
                activeTab === item.id 
                  ? 'bg-slate-50 text-slate-900' 
                  : 'text-slate-400 hover:text-slate-900 hover:bg-slate-50/50'
              }`}
            >
              <div className={`p-2 rounded-xl transition-all duration-300 ${activeTab === item.id ? 'bg-white shadow-sm' : 'group-hover:bg-white/50'}`}>
                {item.icon}
              </div>
      ...
      <div className="mt-auto p-8">
        <div className="bg-slate-50 p-5 rounded-3xl border border-slate-100 flex items-center space-x-3 hover:bg-slate-100 cursor-pointer transition-colors">
          <div className="relative">
            <img src="https://picsum.photos/seed/maya/200/200" className="w-10 h-10 rounded-full border-2 border-white shadow-sm" alt="Maya Sol" />
